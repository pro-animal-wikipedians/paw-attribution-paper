...
Our attribution experiments used Llama 3.1 8B (TrackStar) and Llama 3.2 1B (MAGIC, ablation). These are smaller than frontier models like GPT-4 or Claude, but given Wikipedia is one of the main corpora models are trained on, these frontier models likely saw Wikipedia entries when they were small too. We demonstrate the mechanism at scales where attribution is computationally feasible; the next section argues that the effect persists, and likely strengthens, at frontier scale. Future work will include strengthening the relationship of influence to model scale and extending this primary result to different model architectures to explore how architecture modifies influence.
...
Second, larger models have greater capacity to retain per-document associations. Empirical work on memorization shows that larger models memorize more training examples, not fewer \citep{carlini2023poisoning}. The influence of a given document may actually grow with model capacity even as the corpus grows, partially offsetting the dilution from more data.
