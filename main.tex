...
\author{Brazilek J \and Navas M \and Gnauck A}
\date{}
...
MAGIC goes beyond retrieval similarity by backpropagating through the full training history. It fine-tunes a model on the document set, records optimizer checkpoints at each step, then traces backwards to compute how much each document's gradient contributed to the model's loss on a given query. The resulting score estimates the counterfactual: how would query loss change if this document were removed from training?

We fine-tuned Llama-3.2-1B with LoRA (rank 32, targeting q\_proj and v\_proj, alpha 64) on the full 236-document dataset (118 AW + 118 WikiText-103 controls) for one epoch with a polynomial learning rate schedule (peak lr $= 4 \times 10^{-4}$, 25\% warmup), AdamW optimizer ($\beta_1 = 0.95$, $\beta_2 = 0.975$), weight decay 0.01, batch size 2, and fp32 precision. MAGIC validation uses leave-subset-out retraining with 10 subsets to verify that predicted influence scores correlate with actual counterfactual loss changes. We ran MAGIC twice: once with the 118 AW sections as queries and once with the 118 control sections as queries.
...
This pattern disappeared for general queries about the same companies (e.g., ``Aldi'' rather than ``Aldi animal welfare''): AW sections made up only 51.8\% of top-5 results ($p = 0.27$), with no significant enrichment at any threshold. The model treats PAW content as relevant specifically when asked about animal welfare---not when asked about the same companies for other reasons. This rules out the possibility that the signal comes from PAW sections simply being longer or better written.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig1_trackstar.png}
    \caption{TrackStar attribution enrichment. PAW-edited sections dominate top results for animal welfare queries (blue) but not general queries (pink). Dashed line shows chance level (50\%).}
    \label{fig:trackstar}
\end{figure}

\subsection{MAGIC Training Influence Attribution}

MAGIC goes further than TrackStar by tracing influence through the full training process, estimating how much each document causally contributes to the model's predictions. We fine-tuned Llama-3.2-1B on the full 236-document dataset (118 AW sections from 108 PAW-edited pages + 118 WikiText-103 controls) and measured how much each document influenced prediction quality.

Figure~\ref{fig:magic} plots each training document's MAGIC influence score for animal welfare queries ($x$-axis) against general queries ($y$-axis). PAW-edited sections describing concrete corporate commitments---Eataly signing the European Chicken Commitment, Kraft Heinz committing to source higher-welfare breeds, Kroger pledging cage-free eggs, Olive Garden phasing out battery cages---cluster on the far right, showing strong selective influence on animal welfare predictions with little effect on general queries. By contrast, PAW edits that simply report scorecard ratings (``received a score of 0 out of 100 from the Humane World Action Fund'') or describe political positions had minimal influence on either query type. Edits describing broken commitments, such as Chick-fil-A backtracking on its cage-free pledge, also showed low influence.

The pattern suggests that the model absorbs specific, action-oriented welfare language more readily than evaluative or political framing. This is a practical finding for advocates: Wikipedia edits describing what companies have committed to do appear to shape LLM outputs more than edits reporting how organizations have rated them.

MAGIC validation through leave-subset-out retraining (10 subsets) confirmed these scores are reliable (Spearman $\rho = 0.81$ for animal welfare queries, $\rho = 0.95$ for general queries, $p \leq 0.005$).

Notably, several control sections also showed high influence on animal welfare queries. These were not random: the most influential control documents came from the same Wikipedia pages as high-influence PAW edits. Darden Restaurants' ``Red Lobster sale'' section, Eataly's ``Origin'' section, and The Cheesecake Factory's ``Founding and early years'' all appeared among the top-scoring controls for animal welfare queries---each sharing a page with a PAW-edited animal welfare section. One control section, Marine shrimp farming's ``Slaughter methods,'' contained welfare-relevant content (describing concerns about shrimp slaughter practices) despite not being a PAW edit. This pattern suggests that PAW edits do more than make their own sections influential: they create entity-level associations in the model, so that other content about the same companies gets pulled into animal welfare contexts too. A single PAW edit on the Eataly page makes the model associate \emph{all} Eataly content more strongly with animal welfare. This amplification effect means that 125 edits may have a larger footprint than the raw document count suggests.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/fig2_magic_influence.png}
    \caption{MAGIC counterfactual influence scores for 72 training documents. Each point is one document; $x$-axis shows influence on animal welfare queries, $y$-axis on general queries. Dark circles are PAW edits describing concrete welfare commitments; light squares are PAW edits with scorecard ratings or political framing; grey circles are control sections. Concrete commitments (right side) selectively influence animal welfare predictions without affecting general knowledge, while scorecard-style edits (left side) have little influence on either.}
    \label{fig:magic}
\end{figure}
...
Our attribution experiments used Llama 3.1 8B (TrackStar) and Llama 3.2 1B (MAGIC, ablation). These are smaller than models like GPT-4 or Claude, but the key point is that those frontier models were also trained on Wikipedia---meaning PAW's edits are already in their training data. We demonstrate the mechanism at scales where attribution is computationally feasible; the next section argues that the effect persists, and likely strengthens, at frontier scale. The MAGIC and ablation experiments used all 125 PAW edits (118 after cleaning); TrackStar used a 36-pair within-article subset. Future work with topic-matched controls---animal welfare text not written by PAW---could tease apart whether the signal comes from PAW's specific framing or from animal welfare content more broadly. We also measure influence on model prediction quality, not on what a chatbot actually says in conversation; bridging that gap is important future work.
...
Second, model scaling provides a structural counterweight to dataset dilution. While the global influence of a document follows an $O(1/N)$ decay, empirical research into the ``long tail'' of data distributions suggests that memorization is a prerequisite for generalization \citep{feldman2020does}. Larger models possess the increased capacity required to retain these sparse, per-document associations. As shown by \citet{carlini2023poisoning}, larger models exhibit higher rates of verbatim memorization, effectively ``locking in'' unique information that would otherwise be lost in smaller architectures. This suggests that for rare or high-utility data, the increase in model capacity can keep local influence near $O(1)$, effectively offsetting the dilution caused by a growing corpus.
